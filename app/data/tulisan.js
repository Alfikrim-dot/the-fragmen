export const tulisan = [
  {
    id: 1,
    title: "Arkais: Jiwa yang Terbelah",
    status: "ongoing",
    excerpt: "Sebuah perjalanan tentang diri yang retak dan kehilangan arah...",
    content: `Ada sesuatu dalam diriku yang tidak pernah benar-benar lahir. 
      Ia tidak memiliki awal yang jelas, juga tidak pernah meminta untuk dipahami. 
      Namun ia ada—menetap, diam, dan perlahan mengambil bentuk dari hal-hal yang tidak pernah sempat kusembuhkan.

      Aku tumbuh dengan keyakinan bahwa menjadi utuh adalah tujuan. 
      Bahwa suatu hari nanti, semua bagian dari diriku akan menyatu dengan sendirinya. 
      Namun semakin aku berjalan, semakin aku menyadari bahwa aku tidak pernah benar-benar utuh.

      Ada bagian yang tertinggal di masa lalu. 
      Ada bagian yang hilang karena terlalu sering mengalah. 
      Dan ada bagian yang mati tanpa pernah sempat dikenal.

      Aku tidak retak dalam satu waktu. 
      Aku terbelah secara perlahan—melalui harapan yang tidak terpenuhi, 
      melalui ekspektasi yang terlalu tinggi, 
      dan melalui diam yang kupaksakan agar semuanya tetap terlihat baik-baik saja.

      Orang-orang melihatku sebagai seseorang yang tetap berjalan. 
      Mereka tidak tahu bahwa setiap langkah yang kuambil adalah hasil dari kompromi dengan diriku sendiri. 
      Bahwa di balik keputusan-keputusan yang terlihat rasional, ada konflik yang tidak pernah selesai.

      Aku belajar menyembunyikan keretakan itu. 
      Bukan karena aku kuat, tapi karena aku tidak tahu bagaimana cara memperbaikinya. 
      Pada akhirnya, aku hanya belajar untuk hidup berdampingan dengan sesuatu yang tidak pernah benar-benar bisa kupahami.

      Ada hari-hari di mana aku merasa baik-baik saja. 
      Namun ada juga hari di mana semua kembali terasa asing—seolah aku hanyalah seseorang yang kebetulan berada di dalam tubuh ini.

      Dan mungkin, selama ini aku salah. 
      Mungkin tujuan hidup bukan untuk menjadi utuh. 
      Mungkin kita memang ditakdirkan untuk membawa bagian-bagian yang tidak pernah selesai.

      Bukan untuk diperbaiki, tapi untuk diterima.

      Karena pada akhirnya, yang membuat kita tetap berjalan bukanlah keutuhan— 
      melainkan kemampuan untuk hidup dengan segala yang telah terbelah.`,
    cover: "/images/Arkais_Jiwa.png",
    pdf: "/dokumen/Arkais_Jiwa.pdf",
  },
  {
    id: 2,
    title: "Ilusi Kendali",
    status: "ready",
    excerpt:
      "Memahami Batas, Melepaskan Beban, dan Menemukan Kendali yang Sebenarnya",
    content: `Kita tidak pernah memilih di mana kita dilahirkan.

      Kita tidak memilih keluarga yang pertama kali membentuk cara kita melihat dunia.

      Kita juga tidak memilih pengalaman-pengalaman awal yang diam-diam menanamkan luka, keyakinan, dan ketakutan di dalam diri kita.

      Namun anehnya,
      kita tetap tumbuh dengan perasaan seolah-olah semua ini adalah hasil dari pilihan kita sendiri.

      Seolah kita yang sepenuhnya menentukan arah,
      seolah kita yang memegang kendali penuh atas hidup ini.

      Padahal, jika kita melihat lebih dalam,
      banyak hal yang membentuk kita justru datang tanpa izin.

      Tanpa pilihan. Tanpa kendali.

      Dan mungkin,
      di situlah letak ilusi terbesar yang kita percayai selama ini.

      Bahwa kita mengendalikan segalanya.

      Padahal, yang kita genggam hanyalah sebagian kecil,
      dari sesuatu yang jauh lebih luas dari diri kita.`,
    cover: "/images/Ilusi_kendali.png",
    pdf: "/dokumen/Ilusi_Kendali_.pdf",
  },
   {
    id: 3,
    title: "Kitab Logika Gelap — Vol. I: Noetika Obscura",
    status: "ongoing",
    excerpt:
      "Sebuah eksplorasi tentang pikiran, kesadaran, dan ilusi realitas yang tidak pernah benar-benar dipahami.",

    content: `PROLOG — MANUSKRIP YANG TERTINGGAL

Tidak semua pemikiran diciptakan untuk dipahami.

Sebagian di antaranya muncul dari celah—dari ruang kosong di antara kesadaran dan realitas yang tidak pernah sepenuhnya selaras.

Tulisan ini bukan untuk menjelaskan.
Ia tidak menawarkan jawaban, juga tidak berusaha menenangkan.

Ia hanya membuka sesuatu yang selama ini tertutup.

Dan mungkin,
seharusnya tetap tertutup.

---

DOKTRIN I — PIKIRAN YANG TIDAK PERNAH MILIKMU

Apa yang kau sebut sebagai “pikiranmu”,
mungkin bukan milikmu.

Ia terbentuk dari hal-hal yang tidak kau pilih:
lingkungan,
pengalaman,
luka,
dan kebetulan yang tidak pernah kau sadari.

Kau tidak menciptakan cara berpikirmu.
Kau hanya mewarisinya.

Dan selama ini,
kau hidup di dalam sesuatu yang tidak pernah benar-benar kau pahami.

---

DOKTRIN II — KESADARAN SEBAGAI ILUSI TERSTRUKTUR

Kesadaran sering dianggap sebagai pusat kendali.

Padahal,
ia lebih menyerupai hasil akhir dari proses yang tidak terlihat.

Apa yang kau sadari hanyalah sebagian kecil,
dari sesuatu yang jauh lebih kompleks.

Dan yang tidak kau sadari,
justru memiliki pengaruh yang lebih besar.

---

DOKTRIN III — REALITAS YANG DITAFSIRKAN

Realitas tidak pernah benar-benar objektif.

Ia selalu ditafsirkan.

Dipengaruhi oleh:
apa yang kau percayai,
apa yang kau takutkan,
dan apa yang ingin kau lihat.

Kau tidak melihat dunia sebagaimana adanya.

Kau melihat dunia sebagaimana dirimu terbentuk.

---

Tulisan ini tidak akan membawamu pada kesimpulan.

Ia hanya akan membuatmu menyadari satu hal:

Bahwa apa yang selama ini kau anggap pasti,
mungkin hanyalah konstruksi yang belum pernah kau pertanyakan.`,

    cover: "/images/Kitab_Logika_Gelap_vol1",
    pdf: "/dokumen/kitab_logika_gelap_vol1.pdf",
  },
];
